...
# ==============================================================================
# ⚡ কুইক এইচটিটিপি হেলথ চেক (ফাষ্ট লিংক ভ্যালিডেটর)
# ==============================================================================
...
# ==============================================================================
# 🎯 মেইন কন্ট্রোলার (হিস্টরি চেক -> কুইক হেলথ চেক -> অটো রিপেয়ার -> সেভ)
# ==============================================================================
async def main():
    ...
    elif SCAN_MODE in ["REPAIR", "REPAIR_LINKS"]:
        if cat_index >= len(CATEGORIES_LIST):
            cat_index = 0
        target_category_name = CATEGORIES_LIST[cat_index]
        print(f"🛠️ [REPAIR MODE] Checking & fixing dead links in: '{target_category_name}'", flush=True)
    else:
    ...
    # ⚡ ২. কুইক এইচটিটিপি হেলথ চেক (ফাষ্ট ডেড লিংক ফিল্টার ও অটো-রিপেয়ার কিউ তৈরি)
    ...
    if existing_movies:
        ...
        for m in existing_movies:
            ...
            # লিংক ডেড থাকলে প্লে-রাইট দিয়ে পুনরায় লিংক উদ্ধারের জন্য পেজ URL বের করা
            if has_dead_link or SCAN_MODE in ["REPAIR", "REPAIR_LINKS"]:
                slug_name = re.sub(r'[^a-z0-9]+', '-', m["name"].lower()).strip('-')
                potential_url = f"{MAIN_SITE_URL}{slug_name}-download/"
                if potential_url not in repair_candidate_urls:
                    repair_candidate_urls.append(potential_url)
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # যদি কেবল REPAIR মোড না থাকে, তবে ওয়েবসাইট থেকে নতুন পোস্ট খুঁজবে
            if SCAN_MODE not in ["REPAIR", "REPAIR_LINKS"]:
            ...
            # টার্গেট ইউআরএল এর অগ্রাধিকার: ১. ডেড লিংক রিপেয়ার -> ২. নতুন মুভি -> ৩. রানিং সিরিজ
            new_movie_urls = []
            
            # প্রথমে ডেড লিংক রিপেয়ার করা পেজগুলোকে লিস্টে যুক্ত করা
            ...
            # খালি জায়গা থাকলে একদম নতুন পোস্ট যুক্ত করা
            if len(new_movie_urls) < TARGET_LIMIT_MOVIES:
            ...
            # তাও খালি জায়গা থাকলে রানিং ওয়েব সিরিজ রি-চেক করা
            if len(new_movie_urls) < TARGET_LIMIT_MOVIES:
            ...
            if new_movie_urls:
                ...
                for movie_url, title, categories, res_list, web_poster in parallel_results:
                    if res_list:
                        ...
                        # যদি মুভি/সিরিজ আগে থেকেই ফাইলে থাকে, ফ্রেশ লিংক মার্জ করা
