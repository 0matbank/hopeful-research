...
# ==============================================================================
# 🎬 সমান্তরাল পাইপলাইন প্রসেসর (উন্নত ওয়াচ বাটন এক্সট্র্যাক্টর)
# ==============================================================================
async def process_movie_parallel_pipeline(browser, movie_url, movie_idx, default_category_name):
    ...
    try:
        ...
        # 🎯 বাটন এক্সট্র্যাক্টর লজিক (উন্নত ও নমনীয়)
        target_buttons = await page.evaluate(r"""
            () => {
                let matches = [];
                let seenUrls = new Set();

                // ১. সকল হেডার এবং ব্লকে রেজোলিউশন টেস্ট
                let headers = Array.from(document.querySelectorAll('h1, h2, h3, h4, h5, h6, .movie-title, .quality-title, .entry-content p, .entry-content div'));
                
                headers.forEach(el => {
                    let headerText = (el.innerText || '').toLowerCase();
                    
                    if (headerText.includes('1080p') || headerText.includes('2160p') || headerText.includes('4k') || headerText.includes('1080')) {
                        ...
                        let searchLimit = 0;
                        while (container && !container.querySelector('a[href*="generate.php"], a.dlbtn') && searchLimit < 5) {
                        ...
                        if (container) {
                            let allBtns = Array.from(container.querySelectorAll('a.dlbtn-watch, a.dlbtn, a[href*="generate.php"]'));
                            let watchBtn = allBtns.find(a => {
                                let txt = (a.innerText || '').toLowerCase();
                                return txt.includes('watch') || txt.includes('online') || a.classList.contains('dlbtn-watch');
                            });
                ...
                // ২. ব্যাকআপ লজিক: যদি হেডারে না পায়, পেজের সব "generate.php" লিঙ্ক সরাসরি টেস্ট
                if (matches.length === 0) {
                    let allWatchAnchors = Array.from(document.querySelectorAll('a[href*="generate.php"], a.dlbtn-watch'));
                    allWatchAnchors.forEach(a => {
                        let href = a.href;
                        if (seenUrls.has(href)) return;

                        let txt = (a.innerText || '').toLowerCase();
                        let parentText = (a.parentElement ? a.parentElement.innerText : '').toLowerCase();
                        let grandParentText = (a.parentElement && a.parentElement.parentElement ? a.parentElement.parentElement.innerText : '').toLowerCase();
                        let combinedText = txt + ' ' + parentText + ' ' + grandParentText;

                        if (combinedText.includes('1080p') || combinedText.includes('2160p') || combinedText.includes('4k') || combinedText.includes('hevc')) {
                            if ((combinedText.includes('720p') || combinedText.includes('480p')) && !combinedText.includes('1080p')) return;
                            
                            seenUrls.add(href);
                            matches.push({
                                button_text: a.innerText.trim(),
                                parent_text: parentText.trim(),
                                url: href
                            });
                        }
                    });
