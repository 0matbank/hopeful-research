...
def is_stream_link_dead_sync(stream_url):
    ...
    check_timeout = 8 if is_cdn else 5
...
def should_skip_repair(movie_name, failed_repairs):
    ...
    if key in failed_repairs:
        last_attempt = failed_repairs[key].get("last_attempt", "")
        if last_attempt:
            try:
...
def record_failed_repair(movie_name, category, dead_links, failed_repairs):
    ...
    failed_repairs[key] = {
        ...
        "attempt_count": failed_repairs.get(key, {}).get("attempt_count", 0) + 1
    }
    ...
    for movie in existing_movies:
        ...
        if dead_indices:
            movies_with_dead_links.append({
                ...
                "dead_links": [res_list[i].get("link", "") for i in dead_indices]
            })
