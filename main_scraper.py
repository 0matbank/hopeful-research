...
# ==============================================================================
# ⚡ কুইক এইচটিটিপি হেলথ চেক (Cloudflare R2 সহ নির্ভুল ভ্যালিডেটর)
# ==============================================================================
def is_stream_link_dead_sync(stream_url):
    ...
    # Cloudflare R2 বা Direct Storage Link গুলোর জন্য
    if any(cdn in clean_url for cdn in ["r2.dev", "cloudflarestorage.com", "r2.cloudflarestorage"]):
        headers = {
...
# ==============================================================================
# 📑 ব্যাকআপ রিডার ও ব্যর্থ রিপেয়ার ট্র্যাকার
# ==============================================================================
...
async def main():
    ...
    # ⚡ ২. কুইক এইচটিটিপি হেলথ চেক (২৪ ঘণ্টার রিপেয়ার কোল্ডডাউন সহ)
    ...
    if existing_movies:
        ...
        for m in existing_movies:
            ...
            # যদি গত ২৪ ঘণ্টায় কোনো মুভির রিপেয়ার ফেইল হয়ে থাকে, আপাতত স্কিপ
    ...
    async with async_playwright() as p:
        ...
        try:
                ...
                for movie_url, title, categories, res_list, web_poster in parallel_results:
                    if res_list:
                        ...
                        if key_name in existing_map:
                            ...
                            # শুধুমাত্র সত্যি কাজ করার মতো নতুন লিংক যুক্ত হবে
                            for new_item in parsed_res_list:
                                if new_item['link'] not in existing_links:
                                    # ওয়েবসাইট থেকেও যদি সেই একই ডেড লিংক আসে, তা ইগনোর করা হবে
                            ...
                            if added_count > 0:
                                ...
                                # নতুন কোনো কাজ করার লিংক না পেলে রিপেয়ার ফেইল্ড ফাইলে সেভ
            ...
            # 🎯 ৬. সাল অনুযায়ী সাজানো (Descending - 2026, 2025, 2024...)
            def get_sort_year(m):
                y = m.get("year", "0")
