...
# ==============================================================================
# 🎬 সমান্তরাল পাইপলাইন প্রসেসর (শুধুমাত্র Watch Online বাটন এক্সট্র্যাক্টর)
# ==============================================================================
async def process_movie_parallel_pipeline(browser, movie_url, movie_idx, default_category_name):
    ...
    try:
        ...
        # 🎯 শুধুমাত্র WATCH ONLINE বাটন ফিল্টার লজিক
        target_buttons = await page.evaluate(r"""
            () => {
                let matches = [];
                let seenUrls = new Set();

                // ১. যদি পেজে .ep-card (সিরিজ পেজ) থাকে, তবে শুধুমাত্র Watch Online বাটন ধরা
                ...
                if (epCards.length > 0) {
                    epCards.forEach(card => {
                        ...
                        // শুধুমাত্র ওয়াচ অনলাইন বাটন ফিল্টারিং (Download বাটন সম্পূর্ণ বাদ)
                        let watchBtns = Array.from(card.querySelectorAll('a.dlbtn-watch, .watch-links a, .quality-box.watch-links a'));
                        ...
                        watchBtns.forEach(a => {
                            ...
                            // Download বাটন স্পষ্ট বাদ দেওয়া
                            if (txt.includes('download') && !txt.includes('watch')) return;
                            
                            let isWatchBtn = a.classList.contains('dlbtn-watch') || txt.includes('watch') || txt.includes('online');
                            if (!isWatchBtn) return;
                    ...
                    // ২. সাধারণ সিঙ্গেল মুভি পেজের জন্য শুধু Watch বাটন ফিল্টার
                    ...
                    headers.forEach(h => {
                        ...
                        if (hText.includes('1080p') || hText.includes('2160p') || hText.includes('4k') || hText.includes('hevc')) {
                            ...
                            let searchLimit = 0;
                            while (container && !container.querySelector('a.dlbtn-watch, a[href*="generate.php"]') && searchLimit < 4) {
                            ...
                            if (container) {
                                let watchBtn = Array.from(container.querySelectorAll('a.dlbtn-watch, a.dlbtn, a[href*="generate.php"]')).find(a => {
                                    let txt = (a.innerText || '').toLowerCase();
                                    return (txt.includes('watch') || txt.includes('online') || a.classList.contains('dlbtn-watch')) && !txt.includes('download');
                                });
