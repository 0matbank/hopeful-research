...
# ==============================================================================
# ⚙️ ১. কনফিগারেশন এবং ক্যাটাগরি তালিকা (Disney+ Hotstar সহ)
# ==============================================================================
...
def parse_existing_output_file(file_path):
    ...
    try:
        ...
        for block in blocks:
            ...
            for line in lines:
                line = line.strip()
                if line.startswith("STREAM Link") or line.startswith("Link-"):
                    curr_link = line.split(":", 1)[1].strip() if ":" in line else line
                    res_links.append({"resolution": "HD 1080P", "link": curr_link})
...
# ==============================================================================
# 🎬 ৬. সমান্তরাল পাইপলাইন প্রসেসর (Strict 1080p+ & '3' Match Marker)
# ==============================================================================
async def process_movie_parallel_pipeline(browser, movie_url, movie_idx, default_category_name):
    ...
    def handle_response(response):
        try:
            ...
            if "r2.dev" in decoded_url or any(ext in decoded_url.lower() for ext in [".mkv", ".mp4", ".m3u8"]):
                pass
    ...
    try:
            ...
            web_poster_url = await page.evaluate(r"""
                () => {
                    function isValidImg(url) {
                        ...
                        if (u.includes('admin-ajax.php') || u.includes('rank_math') || u.includes('logo') || u.endsWith('.gif')) return false;
                    ...
                    for (let img of allImgs) {
                        ...
                        if (src.includes('cineimg.xyz') && isValidImg(src)) return src;
                    }

                    let ogImg = document.querySelector('meta[property="og:image"]');
                    if (ogImg && isValidImg(ogImg.content)) return ogImg.content;

                    let postImgs = Array.from(document.querySelectorAll('.wp-post-image, .post-thumbnail img, article img, .entry-content img'));
                    for (let img of postImgs) {
                        let src = img.src || img.getAttribute('data-src') || "";
        ...
        # 🎯 ৩টি গোল্ডেন রুল ফিল্টার (1080p/4K Header + Watch Online Text + Base64 '3' /x/ Marker)
        target_buttons = await page.evaluate(r"""
            () => {
                let matches = [];
                let seenUrls = new Set();

                // ১. মুভি পেজের জন্য h4.movie-title ফিল্টার
                let dlDivs = Array.from(document.querySelectorAll('.download-links-div'));
                
                dlDivs.forEach(div => {
                    let headers = Array.from(div.querySelectorAll('h4.movie-title'));
                    ...
                    headers.forEach(h4 => {
                        ...
                        if (headerText.includes('1080p') || headerText.includes('2160p') || headerText.includes('4k')) {
                            ...
                            while (container && !container.classList.contains('dlbtn-container')) {
                            ...
                            if (container) {
                                ...
                                if (!watchBtn) {
                                    let allBtns = Array.from(container.querySelectorAll('a.dlbtn'));
                ...
                // ২. যদি সিরিজ কম্বো কার্ড থাকে তবে সেখানেও 1080p+ ও '3' মার্কার ফিল্টার
                if (matches.length === 0) {
...
# ==============================================================================
# 🎯 ৭. মেইন কন্ট্রোলার (পোস্টার ব্যাকফিল ও স্ট্রাকচারড ফরম্যাটিং)
# ==============================================================================
async def main():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if not new_movie_urls:
                ...
                # 📝 ৪. পুরানো + নতুন সব পোস্ট একত্রিত করা
                ...
                # 📝 ৬. ফাইলে ১ থেকে N পর্যন্ত সোর্টকৃত ফরম্যাটে রাইট করা
